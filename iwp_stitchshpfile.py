...
def stitch_shapefile(input_root,input_name,output_root):
    ...
    # save shp file
    output_path_1 = os.path.join(output_root,"%s.shp"%input_name)
    w.save(output_path_1)
    ...
    for row in range(0,len(centroid_list),block_size):
        for col in range(0,len(centroid_list),block_size):
            ...
            filter_object_array = np.argwhere((dst_array<5) & (dst_array!=0))
